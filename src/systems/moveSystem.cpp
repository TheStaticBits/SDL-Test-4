...
namespace Systems
{
	void updateKeyboardInputVels(entt::registry& registry, Window& window)
	{
		...
		{
			auto [mov, acc, staticAcc] = view.get<Comps::Movement, Comps::Acceleration, Comps::StaticAcceleration>(entity);
			
			// Update x and y accelerations
			updateKeyboardAcc(window, dirX, mov.vel.x, acc.acc.x, staticAcc.acc.x); 
			updateKeyboardAcc(window, dirY, mov.vel.y, acc.acc.y, staticAcc.acc.y);
		}
	}

	void updateKeyboardAcc(Window& window, const int& keyInputDir, float& vel, float& acc, float& staticAcc)
	{
		if (keyInputDir != 0) acc = staticAcc * keyInputDir; // Accelerate in the direction of the input
	...
	{
		const auto view = registry.view<Comps::Movement, Comps::Acceleration, Comps::Position, Comps::Hitbox, Tags::ChecksCollisions>();
		for (const entt::entity entity : view)
		{
			auto [mov, acc, pos, hitbox] = view.get<Comps::Movement, Comps::Acceleration, Comps::Position, Comps::Hitbox>(entity);

			pos.pos.x += mov.vel.x * window.getDeltaTime();

			// Check collisions from x movement
			const auto [collisionEntityX, thisBoxX, collidedBoxX] = Systems::checkAllCollisions(registry, pos, hitbox, entity);
			if (collisionEntityX != entity)
			{
			...
			const auto [collisionEntityY, thisBoxY, collidedBoxY] = Systems::checkAllCollisions(registry, pos, hitbox, entity);
			if (collisionEntityY != entity)
			{
				if (mov.vel.y > 0)
				{
			...
			for (Comps::Box parentHitboxBox : hitbox.boxes)
			{
				...
				for (Comps::Box checkingHitboxBox : checkingHitbox.boxes)
				{
					...
					if (Utility::collision(parentHitboxBox, checkingHitboxBox))
						return { checkingEntity, parentHitboxBox, checkingHitboxBox };
				}
			}
		}

		return { entity, Comps::Box(), Comps::Box() }; // No entity collision found
	}
}